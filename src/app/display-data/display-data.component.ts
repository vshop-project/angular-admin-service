...
async loadUserProfile() {
  ...
  if (this.isLoggedIn) {
    ...
    //   this.DeliveryUnitNoti=data.deliveryUnitNoti
    //   this.FleetId=data.fleetId;
    //   if(this.TopicName==this.shopRegId){
    ...
    console.log(headers);
      ...
      this.httpClientService.getEmployees(this.locationid,headers).subscribe(
    ...
    else 
    {
      ...
      this.httpClientService.getEmployees(newlocation,headers).subscribe(
        response =>this.handleSuccessfulResponse(response),
       ); 
       localStorage.setItem('newLocation', '{}'); 
...
removeDuplicates(myArray, Prop) {
  ...
  //  this.selectedLoc.locationName = event.currentTarget.innerText;
   ...
   this.httpClientService.getEmployees(this.selectedLoc.locationName,headers).subscribe(
    response =>this.handleSuccessfulResponse(response),
   ); 
      };
...
//   });
// const data="abc123";
// // return this.http.get('http://localhost:8089/test', { headers })
// this.httpClientService.createKeyclok()
//   .subscribe(
  ...
  async DefaultLoc(event: any){
   ...
   if(this.enteredPrdVale==""){
    ...
    const  locationName="THODUPUZHA";
     this.httpClientService.getEmployees(locationName,headers).subscribe(
      response =>this.handleSuccessfulResponse(response),
     ); 
    }
  } 
  PurchHist(){
this.router.navigate(['/purchHist',this.shopRegId])
  }
